...
| D | テキスト抽出異常 | 中 | 文字間スペース挿入や弧状配置によりテキスト抽出に影響がある |
...
パターン B（ToUnicode CMap 欠落）の検出対象はコンポジットフォント（CID フォント）に限定している。

非コンポジットフォント（単純な TrueType や Type 1）は標準エンコーディングを持つ。WinAnsiEncoding や MacRomanEncoding 等がこれにあたる。標準エンコーディングには暗黙の文字コードマッピングが含まれる。そのため ToUnicode CMap がなくてもテキスト抽出が正常に動作する場合が多い。

一方、コンポジットフォントは Identity-H / Identity-V エンコーディングを使用することが多い。ToUnicode CMap がないとグリフ ID から Unicode への逆変換ができない。コンポジットフォントでの ToUnicode 欠落は文字化けに直結するリスクが高い。
